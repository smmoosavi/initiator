...
use smomoo\util\ResponseException;
...
class Csrf extends \Slim\Middleware
{
    ...
    public function check()
    {
        ...
        if (in_array($this->app->request()->getMethod(), array('POST', 'PUT', 'DELETE'))) {
            ...
            $userToken = $this->app->request()->post($this->key) ? : $env[$normalized_header];
            if ($token !== $userToken) {
                $this->app->error(new ResponseException('Invalid or missing CSRF token.', 400));
            }
        }
